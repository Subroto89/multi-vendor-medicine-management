...
const CatMedicineRow = ({medicine, handleModalView, handleAddToCart}) => {
    // const {user} = useAuth();
    const {_id, medicineName, genericName, company, status, stockQuantity, perUnitPrice} = medicine;
    return (
        <tr className='border-b-1 border-gray-300'>
            ...
            <td className='text-sm text-left px-5 py-1 flex items-center gap-2'>
                ...
                <button onClick={()=>handleAddToCart(stockQuantity, _id)} className='btn btn-outline btn-xs hover:bg-green-500 hover:text-white font-bold'>Select</button>
            </td>
        </tr>
    );
};
