...
const SellerPaymentHistory = () => {
  ...
  return (
    <div className="w-full">
      <div className={`min-h-screen w-full mx-auto shadow-xl ${theme==="dark" ? "dark-bg-body" : "light-bg-body"}`}>
        <div className={`min-h-screen w-11/12 mx-auto py-8 ${theme==="dark" ? "dark-bg-body" : "light-bg-body"}`}>
          <h1 className={`text-3xl font-bold text-gray-800 text-center ${theme==="dark" ? "text-white" : "text-gray-800"}`}>
            Your Sales History
          </h1>
          <div className="flex items-center justify-between"> 
            <p className={`text-lg text-gray-600 text-center ${theme==="dark" ? "text-white" : "text-gray-600"}`}>
            Total orders containing your medicines:{" "}
            <span className={`font-semibold ${theme==="dark" ? "white" : "text-blue-700"}`}>
          ...
          {paymentHistory.length > 0 ? (
            <div className={`max-h-[calc(100vh-226px)] overflow-auto rounded-lg shadow-lg border ${theme==="dark" ? "border-none" : "border-gray-200"}`}>
              <table className="min-w-full divide-y divide-gray-500 ">
                <thead className={`bg-gray-50 sticky top-0 ${theme==="dark" ? "dark-category-card" : "light-category-card"}`}>
                  <tr>
                ...
                <tbody className="bg-white divide-y divide-gray-500">
                  {paymentHistory.map((order, index) => (
                    <tr key={order._id || index} className={`${theme==="dark" ? "dark-category-card" : "light-category-card"}`}>
